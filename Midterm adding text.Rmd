...
#Modelling
```{r splitting data for prediction, include=FALSE}
modellingdata = studentdata.f %>%
  filter(toPredict == 'MODELLING') 
...
In terms of specific amenities, the presence of air conditioning significantly boosts a home’s value, with an increase of around $85,192. Similarly, homes classified as having "good" building quality command an additional price of approximately $116,483, reflecting the high demand for well-constructed properties. However, the inclusion of a basement is associated with a decrease in price of about $25,035, which may suggest that basements in certain neighborhoods do not add value, possibly due to concerns related to flooding or maintenance issues.

The analysis also reveals that single-story homes are linked to a decrease in sale price of roughly $39,965, indicating a preference for multi-story homes in this market. The aesthetic quality of the scenery plays a role as well; while good scenery adds value, standard scenery is associated with a significant reduction in price of about $22,079. This highlights the importance of neighborhood aesthetics in influencing home valuations.
...
Using the baseline model, the sale price by dividing the data into training and testing sets. It is then computed the Mean Absolute Error (MAE) and Mean Absolute Percentage Error (MAPE) for the testing set. Currently, the MAE is 93360.49, while the MAPE stands at 39.67%.
...
A new regression model with the Census indicators is listed below. All indicators show We built a new regression of the model with these indicators and obtained the following results. All indicators show statistically significant correlations with house sale prices at the 95% level, except for the presence of a basement, good scenery and the distance to the nearest hospital. Adjusted R-squared is 0.507, indicating that the model can explain 50.7% of variations in sale price. 

```{r spatial table}
reg2 <- lm(sale_price ~ ., data = studentdata.sp %>% st_drop_geometry() %>% 
                                 dplyr::select(sale_price, age, totRooms, incBasements, incAC, buildingQuality, incGarage, stories, scenery, maxArea, farmersmarket.d.ft, schools.c, hospital.d.ft, bikestations.c, parks.d.ft, minority.m, medHHInc))
...
reg2.sumtable %>%
  ...
  footnote(general_title = "\n", general = "Table 5")
```
Then apply the Mean Absolute Error (MAE) and Mean Absolute Percentage Error (MAPE) for the testing set. Resulting in the MAE is 87171.67, while the MAPE stands at 39.53%.
```{r regression of training model for spatial}
trainingReg.sp <- lm(sale_price ~ ., na.action = na.omit, data = trainingmodel.sp %>% 
                      st_drop_geometry() %>% 
                    dplyr::select(sale_price, age, totRooms, incBasements, incAC, incGarage, buildingQuality, stories, maxArea, farmersmarket.d.ft, schools.c, hospital.d.ft, bikestations.c, parks.d.ft, minority.m, medHHInc))

trainingmodel.sp.m = 
  trainingmodel.sp %>% 
  mutate(Regression = "BASE", 
    SalePrice.Predict = predict(trainingReg.sp, trainingmodel.sp),
         SalePrice.Error = SalePrice.Predict - sale_price,
         SalePrice.AbsError = abs(SalePrice.Predict - sale_price),
         SalePrice.APE = (abs(SalePrice.Predict - sale_price)) / SalePrice.Predict)
...
The predicted prices as a function of observed sale prices is then plotted below. The green line represents a would-be perfect fit, while the dark green line represents the predicted fit. It is demonstrate that with the addition of the Census data, the model does a pretty satisfying job at predicting the sale price, perhaps by explaining part of the spatial process and local amenities.
...
trainingmodel.sp.m %>%
  ...
  stat_smooth(aes(SalePrice.Predict, sale_price), 
              method = "lm", se = FALSE, size = 1, colour="black") +
...
```
#Discussion
This model effectively predicts most housing prices, although a few exceptions—likely attributable to extreme values and the inherent randomness within the data—are noted.
